...
user = pd.read_table("/home/yiwen23/WY-CODE-Upload/scripts/data/douban/douban_dataset(text information)/users_cleaned.txt",sep='\t',header=0)
user  = user[["living_place","join_time","UID"]]
user = user.rename(columns={'UID': 'user_id'})

Book = pd.read_table('/home/yiwen23/WY-CODE-Upload/scripts/data/douban/douban_dataset(text information)/bookreviews_cleaned.txt',sep='\t',header=0)
Book = Book[["user_id","book_id","rating"]]
Book["domain_id"] = 0
Book = Book.rename(columns={'book_id': 'item_id'})

Movie = pd.read_table('/home/yiwen23/WY-CODE-Upload/scripts/data/douban/douban_dataset(text information)/moviereviews_cleaned.txt',sep='\t',header=0)
Movie = Movie[["user_id","movie_id","rating"]]
Movie["domain_id"] = 1
Movie = Movie.rename(columns={'movie_id': 'item_id'})

Music = pd.read_table('/home/yiwen23/WY-CODE-Upload/scripts/data/douban/douban_dataset(text information)/musicreviews_cleaned.txt',sep='\t',header=0)
